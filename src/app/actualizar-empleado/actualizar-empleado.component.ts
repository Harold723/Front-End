...
export class ActualizarEmpleadoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.id_empleado = this.route.snapshot.params['id'];
    this.empleadoServicio.obtenerEmpleadoPorId(this.id_empleado).subscribe(dato => {
      this.empleado = dato;
  ...
  onSubmit() {
    this.empleadoServicio.actualizarEmpleado(this.id_empleado, this.empleado).subscribe(dato => {
  ...
  actualizarEmpleados(): void {
    ...
    this.empleadoServicio.actualizarEmpleado(id, this.empleado).subscribe(
      data => {
        this.router.navigate(["/Sistema/empleado"]);
      }
    )
  }
